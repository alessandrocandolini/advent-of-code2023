...
spec = describe "Day 4" $ do
  ...
  it "parse single card (including double space)"
    ...
    `shouldBe` Just (Card (CardId 1) [41, 48, 83, 86, 17] [83, 86, 6, 31, 17, 9, 48, 53])

  it "parse single card (with no double space)"
    $ parseCard "Card 2: 13 32 20 16 61 | 61 30 68 82 17 32 24 19"
    `shouldBe` Just (Card (CardId 2) [13, 32, 20, 16, 61] [61, 30, 68, 82, 17, 32, 24, 19])
  ...
  it "parse card where winning numbers start in single digit (ie, there is an extra space)"
    ...
    `shouldBe` Just (Card (CardId 3) [1, 21, 53, 59, 44] [69, 82, 63, 72, 16, 21, 14, 1])
  ...
  it "parse card where numbers start in single digit (ie, there is an extra space)"
    ...
    `shouldBe` Just (Card (CardId 3) [1, 21, 53, 59, 44] [9, 82, 63, 72, 16, 21, 14, 1])
  ...
  it "parse full input"
    $ parseInput
     ...
     |]
    `shouldBe` [ Card (CardId 1) [41, 48, 83, 86, 17] [83, 86, 6, 31, 17, 9, 48, 53]
